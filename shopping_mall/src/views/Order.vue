<template>
  <div class="order-box">
    ...
    <van-tabs @change="onChangeTab" :color="'#666600'" :title-active-color="'#666600'" class="order-tab" v-model="status">
    ...
    <van-pull-refresh v-model="refreshing" @refresh="onRefresh" class="order-list-refresh">
      ...
      >
        <div v-for="(item, index) in list" :key="index" class="order-item-box" @click="goTo(item.orderNo)">
          ...
          <van-card
            v-for="one in item.simpleMallOrderItemVOS"
            :key="one.orderId"
            :num="one.goodsCount"
            :price="one.sellingPrice"
            desc="全场包邮"
            :title="one.goodsName"
            :thumb="`//118.178.89.44:28019${one.goodsCoverImg}`"
          />
        </div>
      </van-list>
    </van-pull-refresh>
